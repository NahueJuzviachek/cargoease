...
def _neumaticos_vigentes_qs():
    """
    Devuelve un QS de Neumatico excluyendo 'bajas' o 'eliminados' según campos disponibles.
    (Soporta distintos esquemas sin romper: activo/eliminado/fecha_baja)
    """
...
def data_clientes_top_mensual(request):
    ...
    def _run_period(y1i, m1i, y2i, m2i):
        ...
        series = []
        for cid in top_ids:
            label = labels_map.get(cid) or f"Cliente {cid}"
            series.append({"label": label, "data": series_dict[cid]})
...
def data_aceite_top5(request):
    ...
    items = [{
        "vehiculo": a.get('vehiculo_label') or "",
        "tipo": a.get('tipo') or "",
        "km_restantes": float(a.get('km_restantes') or 0),
    } for a in qs]
...
def data_neumaticos_estado(request):
    # Usa solo neumáticos vigentes (no dados de baja / no eliminados)
